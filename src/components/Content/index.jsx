...
export default function Content() {
  ...
  return (
    <>
      <main
        id="Content"
        className="w-full flex flex-row h-screen fixed top-0 left-0 p-0 md:p-10"
        style={{
          transform: `matrix(1,0,0,1,0,${-fixPage <= -910 ? 0 : -fixPage})`,
          visibility: `${fixPage >= 910 ? "hidden" : "visible"}`,
        }}
      >
        <section className="w-full flex flex-col bg-principal flex-1 pb-10 md:pb-0 justify-end md:justify-center ">
          <About scrolled={scrolled} animDuration={animDuration} />
        </section>
        <FreeArea scrolled={scrolled} animDuration={animDuration} />
        <Welcome
          start={transitionAnimFinish}
          scrolled={scrolled}
